...
- displays 'Closed' if the `closed` prop is `true` and 'Open' if otherwise ✅✅ (two tests) // Display.test.js
- displays 'Locked' if the `locked` prop is `true` and 'Unlocked' if otherwise ✅✅ (two tests) // Display.test.js
- when `locked` or `closed` use the `red-led` class ✅✅ (two tests) // Display.test.js
- when `unlocked` or `open` use the `green-led` class ✅✅ (two tests) // Display.test.js

### Controls Component

- provide buttons to toggle the `closed` and `locked` states. ✅ (as 'renders 2 buttons') // Controls.test.js
